 ...
 * 调用
 * //处理bitmap的轮廓
		HolographicOutlineHelper helper = new HolographicOutlineHelper(getApplicationContext());
		Bitmap bitmap = BitmapFactory.decodeResource(getResources(), R.drawable.ic_launcher);
		
	    final Bitmap b = Bitmap.createBitmap(bitmap.getWidth(), bitmap.getHeight(), Bitmap.Config.ARGB_8888);

		Canvas canvas = new Canvas();
		if(bitmap == null){
			Log.e(TAG, "bitmap is null");
		}
		Log.e(TAG, "bitmap is null" + bitmap.isMutable());
		canvas.setBitmap(b);
		canvas.drawBitmap(bitmap, new Matrix(), new Paint());
		helper.applyExtraThickExpensiveOutlineWithBlur(b, canvas, Color.WHITE, Color.WHITE);
...
public class HolographicOutlineHelper {
    ...
    public static int MIN_OUTER_BLUR_RADIUS;

    private static BlurMaskFilter sExtraThickOuterBlurMaskFilter;
    private static BlurMaskFilter sThickOuterBlurMaskFilter;
    private static BlurMaskFilter sMediumOuterBlurMaskFilter;
    private static BlurMaskFilter sThinOuterBlurMaskFilter;
    private static BlurMaskFilter sThickInnerBlurMaskFilter;
    private static BlurMaskFilter sExtraThickInnerBlurMaskFilter;
    private static BlurMaskFilter sMediumInnerBlurMaskFilter;
    ...
    HolographicOutlineHelper(Context context) {
